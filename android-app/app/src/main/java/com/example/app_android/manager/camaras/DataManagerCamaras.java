package com.example.app_android.manager.camaras;
...
public class DataManagerCamaras {
    ...
    public CompletableFuture<List<DataItem>> loadMyApiData(String jwtToken) {
        ...
        Request request = new Request.Builder().url("http://10.0.2.2:8080/api/camaras/seleccionar").addHeader("Authorization", "Bearer " + jwtToken).build();
    ...
    private List<DataItem> parseItems(String responseBody) {
        ...
        if (jsonElement != null && jsonElement.isJsonArray()) {
            ...
            Log.d("DataManagerCamaras", "No hay datos o el JSON no contiene un array");
        }
        return items;
    }
    ...
    public CompletableFuture<String> insertarCamara(String jsonCamara, String jwtToken) {
        CompletableFuture<String> future = new CompletableFuture<>();
        String url = "http://10.0.2.2:8080/api/camaras/insertar";

        // Crear el cuerpo de la solicitud
        RequestBody body = RequestBody.create(jsonCamara, JSON);
        ...
        client.newCall(request).enqueue(new Callback() {
            ...
            public void onResponse(Call call, Response response) throws IOException {
                if (response.isSuccessful()) {
                    future.complete("Camara insertada con éxito");
                } else {
                    future.completeExceptionally(new IOException("Error al insertar camara. HTTP status: " + response.code()));
                }
            }
        });
